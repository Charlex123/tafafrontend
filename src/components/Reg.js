...
//   const isConnected = Boolean(accounts[0]);
  ...
  const checkUsername = async (e) => {
    ...
    const {data} = await axios.post("https://betbackend.onrender.com/api/users/checkusername", {
          username,
    }, config);
    if(data) {
      setLoading(false);
  ...
  const checkEmail = async (e) => {
    ...
    const {data} = await axios.post("https://betbackend.onrender.com/api/users/checkemail", {
          email,
    }, config);
    if(data) {
      setLoading(false);
